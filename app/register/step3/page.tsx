...
const goalOptions = [
  { value: 1, label: "1 个月", emoji: "🌱", days: 30 },
  { value: 3, label: "3 个月", emoji: "🌿", days: 90 },
  { value: 6, label: "6 个月", emoji: "🌳", days: 180 },
];
...
export default function RegisterStep3() {
  ...
  const weightDiff = currentWeight && targetWeight ? currentWeight - tw : null;
  const selectedDays = goalOptions.find(o => o.value === goalMonths)?.days ?? null;
  const rawDeficit = weightDiff && weightDiff > 0 && selectedDays
    ? Math.round((weightDiff * 7700) / selectedDays) : null;
  const dailyDeficit = rawDeficit !== null ? Math.min(rawDeficit, 500) : null;
  const tooAggressive = rawDeficit !== null && rawDeficit > 500;

  function handleNext() {
    setError("");
    if (!targetWeight) { setError("请输入目标体重"); return; }
    if (!goalMonths) { setError("请选择期望达成时间"); return; }
    saveTempData({ targetWeight: tw, goalMonths });
      ...
      <div style={{ maxWidth: "430px", width: "100%" }}>
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          ...
          <div style={{ display: "flex", gap: "6px", marginTop: "14px", justifyContent: "center" }}>
            {[1,2,3,4].map(s => <div key={s} style={{ height: "4px", width: "36px", borderRadius: "4px", background: s <= 3 ? "#C084FC" : "#F3E8FF" }} />)}
          </div>
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: "20px", padding: "28px 24px", boxShadow: "0 4px 24px rgba(192,132,252,0.1)", display: "flex", flexDirection: "column", gap: "20px" }}>
          <div>
            ...
            <input type="number" placeholder="例如：55" value={targetWeight}
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              {goalOptions.map(opt => (
                <button key={opt.value} onClick={() => setGoalMonths(opt.value)} style={{ flex: 1, padding: "16px 8px", borderRadius: "14px", border: `2px solid ${goalMonths===opt.value?"#C084FC":"#F3E8FF"}`, background: goalMonths===opt.value?"rgba(192,132,252,0.1)":"#fff", cursor: "pointer", textAlign: "center", transition: "all 0.2s" }}>
                  <div style={{ fontSize: "22px", marginBottom: "4px" }}>{opt.emoji}</div>
                  <div style={{ fontSize: "13px", fontWeight: goalMonths===opt.value?700:500, color: goalMonths===opt.value?"#C084FC":"#3B1F5E" }}>{opt.label}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* 动态热量提示 */}
          {dailyDeficit !== null && weightDiff !== null && weightDiff > 0 && (
            <div style={{ borderRadius: "12px", padding: "14px 16px", textAlign: "center", background: tooAggressive ? "rgba(251,146,60,0.1)" : "rgba(192,132,252,0.08)" }}>
              {tooAggressive ? (
                <p style={{ margin: 0, fontSize: "14px", color: "#F97316", lineHeight: "1.6" }}>
                  建议放慢节奏，健康优先 🌸<br />
                  <span style={{ fontSize: "12px", color: "#9CA3AF" }}>每日减少热量建议不超过 500 kcal</span>
                </p>
              ) : (
          ...
          {weightDiff !== null && weightDiff <= 0 && targetWeight && (
            <div style={{ background: "rgba(249,168,212,0.15)", borderRadius: "12px", padding: "14px 16px", textAlign: "center" }}>
              <p style={{ margin: 0, fontSize: "14px", color: "#3B1F5E" }}>保持健康体重也是很棒的目标 🌸</p>
            </div>
          )}
